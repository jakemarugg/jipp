...
public class NameTest {
    // TODO: We still need to get attribute types plunked into `Types.all` somehow so that Finder can get them
    ...
    public void withoutLanguage() throws IOException {
        ...
        assertEquals(Tag.nameWithoutLanguage, jobNameAttr.getValueTag());
    ...
    public void withLanguage() throws IOException {
        ...
        assertEquals(Tag.nameWithLanguage, jobNameAttr.getValueTag());
        Attribute<Name> result = cycle(jobNameType, jobNameAttr);
        assertEquals("my job", result.getValues().get(0).getValue());
    ...
    public void mixed() throws IOException {
        jobNameType.of(new Name("my job", "en"), new Name("another job"));
    }

    @Test(expected = BuildError.class)
    public void overlong() throws IOException {
        jobNameType.of(StringsKt.repeat("repeated string ", 100));
    }
}
